// Profile tab — temporary location for the session info / sign out.
// Will eventually become "Friends" or "Account" once we have more screens.
...
export default function ProfileScreen() {
    ...
    (async () => {
      ...
      if (sessionData.session) {
        const { data, error } = await supabase
          ...
          .single();
        if (!mounted) return;
        // PGRST116 = "no rows found" — happens momentarily if signed in
        // on a stale session whose user has since been deleted. We just
        // render placeholders rather than spamming an error toast.
  ...
  const signOut = async () => {
    ...
    // _layout's auth gate redirects to /login automatically.
  };
